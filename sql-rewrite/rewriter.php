...
function rewrite($s, $t, $p, $settings = array()) {
	$planner = new SQLrw($s, $t, $p, $settings);
	return array($planner->errors, $planner->plan);
}
...
class SQLrw {
	...
	public function __construct($sql, $table_info, $process_info, $settings=array()) {
		return $this->rewrite($sql, $table_info, $process_info, $settings);
	}
	...
	public function rewrite($sql, $table_info, $process_info, $settings = array()) {
		...
		$this->create_sql .= " ENGINE=". $this->engine;
