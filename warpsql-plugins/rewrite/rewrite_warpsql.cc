...
static PSI_memory_key key_memory_rewrite_warpsql;

static PSI_memory_info all_rewrite_memory[]=
{
  { &key_memory_rewrite_warpsql, "rewrite_warpsql", 0 }
};

static int plugin_init(MYSQL_PLUGIN)
{
  const char* category= "rewrite";
  int count;
...
#define key_memory_rewrite_warpsql PSI_NOT_INSTRUMENTED
  ...
  if (event_class == MYSQL_AUDIT_PARSE_CLASS)
  {
    ...
    if (event_parse->event_subclass == MYSQL_AUDIT_PARSE_PREPARSE)
    {
      ...
      char *rewritten_query=
        static_cast<char *>(my_malloc(key_memory_rewrite_warpsql,
...
static struct st_mysql_audit rewrite_warpsql_descriptor=
{
  MYSQL_AUDIT_INTERFACE_VERSION,                    /* interface version */
  NULL,                                             /* release_thd()     */
  parallel_rewrite,                                 /* event_notify()    */
  { 0,
    0,
...
mysql_declare_plugin(audit_log)
{
  ...
  &rewrite_warpsql_descriptor,    /* type specific descriptor      */
  "rewrite_warpsql",              /* plugin name                   */
  "Oracle",                       /* author                        */
  "WarpSQL query rewrite plugin"
