...
switch($config['mapper']) {
	case 'directory':
		$mapper = new DirectoryShardKeyMapper($config,false,true);	
	break;
...
$mapper->conn->my_query('DELETE IGNORE cs.* from shard_columns cs join schemata on cs.schema_id = schemata.id where schema_name = "' . $config['schema_name'] . '"')
...
foreach($config as $var_name => $var_value) {
	if(in_array($var_name, array( 'user', 'db', 'host', 'port', 'password', 'schema_name','root_user','root_password' ))) continue;
	if($var_name == 'lookup_db') $lookup_db = $var_value;	
...
foreach($shards as $shard_name => $shard) {
	...
	if($create_user) {
		...
		if($lookup_db) {
			$sql = 'CREATE DATABASE IF NOT EXISTS ' . $lookup_db;
			$dal->my_query($sql);
			if($dal->my_error()) {
				die("SETUP FAILED: Could not create database on $shard_name\n" . $dal->my_error() . "\n");
			}
		  $sql = "GRANT ALL ON " . $lookup_db . ".* TO '" . $shard['user'] . "'@'%'"; 
  		$dal->my_query($sql);
  		if($dal->my_error()) {
