...
-- use the default virtual schema name by default
-- don't change this unless you have multiple virtual
-- schema and want to point the proxy at a particular one
virtual_schema_name=""

function read_query(packet)
  read_query_error=false

  -- All connections get pushed into the INFORMATION_SCHEMA
  -- which is guaranteed to exist and be accessible to all
  -- clients.
  if packet:byte() == proxy.COM_INIT_DB then
    virtual_schema_name = packet:sub(2)
    proxy.queries:append(99999, string.char(proxy.COM_QUERY) .. "use INFORMATION_SCHEMA;", {resultset_is_needed = true} );
    return proxy.PROXY_SEND_QUERY
  end

  if packet:byte() == proxy.COM_QUERY then
    v_sql = string.gsub(packet:sub(2), "'", "''")
    v_sql = string.gsub(v_sql, "\n", " ")
    arg = encode( { sql = v_sql, schema_name = virtual_schema_name } )
...
function read_query_result(inj) 
  if inj.id == 10 then
    ...
    result = decode(string.gsub(string.sub(data,12,-3),'\\"','"'))
